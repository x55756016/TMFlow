    ...
    public class Service : IService
    {
        ...
        public DataResult SubimtFlow(SubmitData submitData)
        {
            ...
            using (TransactionScope ts = new TransactionScope(TransactionScopeOption.Required, new TimeSpan(DateTime.Now.AddMinutes(2).Ticks)))
            {
                ...
                Tracer.Debug("打开数据库 完成  FormID=" + submitData.FormID);
                #region 事务处理
                #region 初始化流程所属人的信息
                ...
                try
                {
                    ...
                    if (submitData.SubmitFlag == SubmitFlag.New)
                    {
                        ...
                        if (submitData.FlowSelectType == FlowSelectType.FreeFlow)
                            //自选流程
                            dataResult = Flowbill.AddFreeFlow( submitData, dataResult, ref User);//对数据库操作
                        else
                        {
                            //固定流程

                            Tracer.Debug("固定流程.Flowbill.AddFlow2");
                            dataResult = Flowbill.AddFlow2( submitData, dataResult, ref User);//对数据库操作
                    ...
                    else
                    {
                        ...
                        if (submitData.FlowSelectType == FlowSelectType.FreeFlow)
                            dataResult = Flowbill.ApprovalFreeFlow( submitData, dataResult, CheckFlowResult.fd, ref  User);//对数据库操作、对服务操作
                        else
                            dataResult = Flowbill.ApprovalFlow2( submitData, dataResult, CheckFlowResult.fd, ref User, ref returnMsg);
                        #endregion
                    }
                    ...
                    if (string.IsNullOrEmpty(strIsFlowEngine) || strIsFlowEngine.ToLower() == "true")
                    {
                        ...
                        SubmitEngineService( submitData, dataResult, ref User, CheckFlowResult, Flowbill, AppCompanyID, MessageUserID, MessageUserName, ref returnMsg);
                        #endregion
                    ...
                    //{
                    //    EnginFlowBLL FlowEngine = new EnginFlowBLL();
                    //    FlowEngine.FlowConsultatiCloseAll( submitData.ModelCode, submitData.FormID);
                    //}
                    #endregion
                ...
                catch (Exception ex)
                {
                    ...
                    {                       
                        foreach (DataRow dr in User.InstanceState.Tables[0].Rows)
                        {
                            ...
                            //Tracer.Debug("恢复运行工作流InstanceState:" + dr["instance_id"]);
