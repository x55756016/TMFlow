...
namespace SMT.FlowWFService
{
    ...
    class SMTWorkFlowManage
    {
        ...
        {
            Tracer.Debug("WfRuntime_ServicesExceptionNotHandled异常信息 ：" + e.Exception.ToString ());
        }

        static void WfRuntime_WorkflowPersisted(object sender, WorkflowEventArgs e)
        {
            Tracer.Debug("WfRuntime_WorkflowPersisted Persisted completed信息 ：" + e.WorkflowInstance.InstanceId.ToString());
        }
        ...
        public static WorkflowInstance GetWorkflowInstance(WorkflowRuntime WfRuntime, string INSTANCEID)
        {
            ...
            {
                Tracer.Debug("GetWorkflowInstance异常信息 ：" + ex.ToString());
                throw new Exception(ex.Message);
            }
        }
        /// <summary>
