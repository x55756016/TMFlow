...
namespace WFTools.Services.Persistence
{
    ...
    public abstract class GenericPersistenceService : WorkflowPersistenceService, IPendingWork, IDisposable
    {
        ...
        protected override void SaveWorkflowInstanceState(Activity rootActivity, bool unlock)
        {
            ...
            {
                Tracer.Debug("持久化SaveWorkflowInstanceState发生异常:" + e.ToString());
                string errorMessage = RM.Get_Error_PersistenceServiceException(e.ToString());
        ...
        protected override Activity LoadWorkflowInstanceState(Guid instanceId)
        {
            ...
            {
                Tracer.Debug("加载工作流状态实例发生异常:LoadWorkflowInstanceState("+instanceId+");异常信息:"+e.ToString());
        ...
        public void Commit(Transaction transaction, ICollection items)
        {
            ...
            try
            {
                ...
                Tracer.Debug("持久化提交事务Commit完成......");
            }
            catch (Exception e)
            {
                Tracer.Debug("持久化提交事务Commit发生异常:" + e.ToString());
                string errorMessage = RM.Get_Error_PersistenceServiceException(e.ToString());
        ...
        public void Complete(bool succeeded, ICollection items)
        {
            ...
            try
            {
                if (succeeded)
                {
                    ...
                    Tracer.Debug("持久化提交事务Complete完成......");
                }
            }
            catch (Exception e)
            {
                Tracer.Debug("持久化Complete务发生异常:" + e.ToString());
                string errorMessage = RM.Get_Error_PersistenceServiceException(e.ToString());
