...
namespace SMT.Workflow.Engine.Services
{
    ...
    public class PersonalRecordService : IPersonalRecordService
    {
        ...
        public bool AddPersonalRecords(List<T_PF_PERSONALRECORD> models)
        {
            ...
            Tracer.Debug("新增AddPersonalRecords(2):SYSTYPE:" + models[0].SYSTYPE + "||MODELCODE:" + models[0].MODELCODE + "||MODELID:" + models[0].MODELID + "||OWNERID" + models[0].OWNERID + "||转发ISFORWARD:" + models[0].ISFORWARD + "");
            PersonalRecordBLL bll = new PersonalRecordBLL();
            ...
            string BeginDate, string EndDate, ref int pageCount)
        {
           ...
           "调用方法GetPersonalRecord（）||pageIndex:" + pageIndex + "||strOrderBy:" + strOrderBy + "||checkstate:" + checkstate + "||filterString:" + filterString + "||strCreateID:" + strCreateID + "  ");
            ...
            {
                Tracer.Debug("调用方法GetPersonalRecord（）||strCreateID:" + strCreateID + " ");
                return null;
            }
        ...
        {
            PersonalRecordBLL bll = new PersonalRecordBLL();
            Tracer.Debug("调用方法GetPersonalRecordList（）||pageIndex:" + pageIndex + "||strOrderBy:" + strOrderBy + "||checkstate:" + checkstate + "||filterString:" + filterString + "||strCreateID:" + strCreateID + "  ");
            if (string.IsNullOrWhiteSpace(strCreateID))
            {
                Tracer.Debug("调用方法GetPersonalRecordList（）||strCreateID:" + strCreateID + " ");
                return null;
            }
